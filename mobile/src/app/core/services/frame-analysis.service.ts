...
@Injectable({providedIn:'root'}) export class FrameAnalysisService {
  measure(current: ImageData, previous?: ImageData, minimumSharpness:number=visionConfig.minimumSharpness): FrameMetrics {
    ...
    return {brightness,variance,sharpness,motionScore,hasContent:variance>=visionConfig.minimumVariance,acceptableExposure:brightness>=visionConfig.minimumBrightness&&brightness<=visionConfig.maximumBrightness,sharp:sharpness>=minimumSharpness};
  }
