import cors from 'cors'; import express from 'express'; import path from 'node:path';
...
export const createApp = (env: Environment, db: Pool) => {
  const app = express(); app.set('trust proxy', 1); app.disable('x-powered-by'); app.use(cors({ origin: env.CORS_ORIGIN.split(',').map(v => v.trim()) })); app.use(express.json({ limit: '50kb' }));
  ...
  app.use('/api/v1/vision', createVisionRouter(new VisionController(service, repository), env));
  app.use('/api', (_req, res) => res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Ruta no encontrada.' } }));
  const publicPath = path.resolve('public'); app.use(express.static(publicPath)); app.get('*', (_req, res) => res.sendFile(path.join(publicPath, 'index.html')));
